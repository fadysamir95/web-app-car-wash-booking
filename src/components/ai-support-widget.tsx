      ...
      {open ? (
        <div className="mb-3 w-[min(92vw,360px)] rounded-[8px] bg-white p-4 text-slate-950 shadow-2xl ring-1 ring-slate-200 dark:bg-slate-900 dark:text-white dark:ring-slate-800">
          ...
          <form onSubmit={ask} className="mt-3 grid gap-2">
            <textarea className="field min-h-20" value={message} onChange={(event) => setMessage(event.target.value)} placeholder={language === "ar" ? "اسأل عن الحجز أو السعر أو حالة الحجز" : "Ask about booking, pricing, or booking status"} />
          ...
          {answer ? <p className="mt-3 rounded-[8px] bg-slate-50 p-3 text-sm font-bold leading-6 dark:bg-slate-800">{answer}</p> : null}
        </div>
      ) : null}
